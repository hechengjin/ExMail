...
const _Change2IconLock = traylib.declare(
    "mintrayr_Change2IconLock",
    abi_t,
    ctypes.int,
    handle_t
  );

const _Change2IconNewMail = traylib.declare(
    "mintrayr_Change2IconNewMail",
    abi_t,
    ctypes.int,
    handle_t
  );

const _RestoreIcon = traylib.declare(
    "mintrayr_RestoreIcon",
    abi_t,
    ctypes.int,
    handle_t
  );
...
const minimize_callback = minimize_callback_t(function minimize_callback(handle, type) {
  try {
    for (let [,w] in Iterator(_watchedWindows)) {
      if (ptrcmp(w.handle, handle)) {
        if (!type) {
          TrayService.minimize(w.window, false);
        }
        else {
...
TrayIcon.prototype = {
  ...
  get isMinimized() this._minimized,
    ...
    change2IconLock: function() {
        _Change2IconLock(this._handle);
    },

    change2IconNewMail: function() {
        _Change2IconNewMail(this._handle);
    },

    restoreIcon: function() {
        _RestoreIcon(this._handle);
    },
...
const TrayService = {
  ...
  showTray: function(window) {
      this.createIcon(window, false);
  },

    change2IconLock: function() {
        for (let [,icon] in Iterator(_icons)) {
            icon.change2IconLock();
        }
    },

    change2IconNewMail: function() {
        for (let [,icon] in Iterator(_icons)) {
            icon.change2IconNewMail();
        }
    },
