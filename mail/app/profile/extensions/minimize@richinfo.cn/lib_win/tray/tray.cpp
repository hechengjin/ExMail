...
BOOL mintrayr_Change2IconLock(void *handle)
{
    HWND hwnd = (HWND)handle;
	if (!hwnd) return FALSE;

	NOTIFYICONDATAW *iconData = reinterpret_cast<NOTIFYICONDATAW *>(::GetPropW(hwnd, kIconData));
	if (iconData == NULL) return FALSE;

	iconData->hIcon = ::LoadIcon(GetModuleHandleW(0), MAKEINTRESOURCE(32575));
	::Shell_NotifyIconW(NIM_MODIFY, iconData);

	return TRUE;
}

BOOL mintrayr_Change2IconNewMail(void *handle)
{
	HWND hwnd = (HWND)handle;
	if (!hwnd) return FALSE;
	...
	iconData->hIcon = ::LoadIcon(GetModuleHandleW(0), MAKEINTRESOURCE(32576));
	::Shell_NotifyIconW(NIM_MODIFY, iconData);

	return TRUE;
}

BOOL mintrayr_RestoreIcon(void *handle)
{
	HWND hwnd = (HWND)handle;
	if (!hwnd) return FALSE;
