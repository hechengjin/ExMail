...
module("chrome://EE/content/mintrayr/modules/platlog.jsm");
module("chrome://EE/content/mintrayr/modules/platdir.jsm");
...
platlog.dumpMsg("_directory  "+Components.stack.filename); // chrome://ee/content/mintrayr/modules/trayservice.jsm
...
function loadLibrary({m,c}) {
  platlog.dumpMsg("loadLibrary m:"+m+"c:"+c);
  //let resource = _directory.clone();
  let resource = platdir.getSpecialDir("bin", "CurProcD");
  
  resource.append("lib");
  resource.append(m);
  platlog.dumpMsg("resource "+resource.path);
  if (!resource.exists()) {
...
var char_ptr_t;
try {
platlog.dumpMsg("loadLibrary "+Services.appinfo.XPCOMABI);
